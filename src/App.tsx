...
export default function App() {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", `civiclink_reports_export_${new Date().toISOString().split('T')[0]}.csv`);
        ...
        {otpSent && simulatedOtp && (
          <div className="max-w-md w-full mb-6 bg-emerald-950/40 border border-emerald-900/60 p-4 rounded-2xl text-center shadow-xl animate-fade-in">
            ...
            <p className="text-xs text-slate-300 font-mono">
              ...
              Your CivicLink OTP secure verification code is <span className="text-emerald-400 font-black text-sm tracking-wider underline">{simulatedOtp}</span>
            </p>
          </div>
        )}
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col font-sans select-none text-slate-200">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-1.5">
              <span className="text-sm font-black text-white tracking-wide uppercase font-mono">CivicLink AI</span>
              <span className="bg-slate-800 text-slate-400 text-[9px] font-mono px-1.5 py-0.5 rounded border border-slate-700">v1.0.0</span>
            </div>
            <span className="text-[10px] text-slate-400 block font-mono">Springfield Municipal Dashboard</span>
          </div>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <nav className="w-64 bg-slate-900 border-r border-slate-800 p-5 flex flex-col justify-between shrink-0">
            ...
            currentUser.role === "SOCIAL_WORKER" ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="lg:col-span-5 space-y-6 lg:sticky lg:top-24">
                  <div className="px-1">
                    <h2 className="text-base font-bold text-white">Issue Inspector</h2>
                    <p className="text-[10px] text-slate-500 font-mono">Select a report card to evaluate parameters</p>
                  </div>

                  {!selectedReport ? (
                    <div className="bg-slate-900 border border-slate-800 p-12 text-center rounded-2xl">
                      <Info className="w-8 h-8 text-slate-700 mx-auto mb-2" />
                      <p className="text-xs text-slate-400">Click on any card to view GPS locations, coordinate tasks, send messages, and evaluate Gemini AI insights.</p>
                    </div>
                  ) : (
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden p-6 space-y-6">
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between items-center">
                          <span className="text-[10px] uppercase font-mono text-slate-400 font-semibold block">Case Photographic Evidence</span>
                          {selectedReport.status === "RESOLVED" && (
                            <span className="text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2 py-0.5 rounded-lg font-mono">
                              Before & After
                            </span>
                          )}
                        </div>
                        ...
                        {selectedReport.status === "RESOLVED" && selectedReport.afterImage ? (
                          <div className="grid grid-cols-2 gap-2">
                            <div>
                              <img src={selectedReport.beforeImage || selectedReport.images[0]} alt="Before" className="w-full h-28 object-cover rounded-xl border border-slate-800" />
                              <span className="text-[9px] uppercase font-mono text-slate-500 text-center block mt-1">Before Work</span>
                            </div>
                            <div>
                              <img src={selectedReport.afterImage} alt="After" className="w-full h-28 object-cover rounded-xl border border-emerald-900" />
                              <span className="text-[9px] uppercase font-mono text-emerald-400 text-center block mt-1">After Resolved</span>
                            </div>
                          </div>
                        ) : (
                          <img src={selectedReport.beforeImage || selectedReport.images[0]} alt="Complaint" className="w-full h-44 object-cover rounded-xl border border-slate-800" />
                        )}
                      </div>
                      ...
                      <div>
                        <div className="flex items-center gap-1.5 mb-1.5">
                          <span className="bg-indigo-600/10 text-indigo-400 text-[10px] font-mono font-bold px-2 py-0.5 rounded border border-indigo-400/20">
                            {selectedReport.category}
                          </span>
                          <span className="text-[11px] text-slate-500 font-mono">ID: #{selectedReport.id}</span>
                        </div>
                        <h3 className="text-sm font-bold text-slate-100">{selectedReport.title}</h3>
                        <p className="text-xs text-slate-400 leading-relaxed mt-2 bg-slate-950/40 p-3.5 rounded-xl border border-slate-800">
                      ...
                      <div className="grid grid-cols-2 gap-3 bg-slate-950/40 p-3 rounded-xl border border-slate-800/60 text-xs">
                        <div>
                          <span className="text-[9px] uppercase font-mono text-slate-500 font-bold block mb-0.5">Reported By</span>
                          <span className="text-slate-200 font-semibold">
                            {usersList.find(u => u.id === selectedReport.citizenId)?.fullName || "Citizen User"}
                          </span>
                        </div>
                        <div>
                          <span className="text-[9px] uppercase font-mono text-slate-500 font-bold block mb-0.5">Assigned / Solver</span>
                      ...
                      {selectedReport.status === "RESOLVED" && (
                        <div className="bg-emerald-950/20 border border-emerald-900/40 p-4 rounded-xl space-y-1.5 text-xs">
                          <span className="text-[10px] uppercase font-mono text-emerald-400 font-bold block">Social Worker Resolution Statement</span>
                      ...
                      <div className="bg-slate-950/60 border border-slate-800/80 p-4 rounded-xl space-y-3">
                        <div className="flex items-center gap-2 text-xs text-slate-300">
                          ...
                          <span className="font-mono">{selectedReport.address}</span>
                        </div>
            ...
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <div className="lg:col-span-4">
                  {!selectedReport ? (
                    <div className="bg-slate-900/60 border border-slate-800/80 p-8 text-center rounded-2xl">
                      <MapPin className="w-8 h-8 text-slate-700 mx-auto mb-2" />
                      <p className="text-xs text-slate-400">Click on any visual marker on the map telemetry to examine details, review severity reports, and verify department assignment.</p>
                    </div>
                  ) : (
                    <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-4 text-left">
                      <span className="text-[9px] uppercase font-mono text-slate-400 font-semibold block">Selected Telemetry Node</span>
                      <h3 className="text-sm font-bold text-slate-100">{selectedReport.title}</h3>
                      <p className="text-xs text-slate-400 leading-normal line-clamp-3">{selectedReport.description}</p>
                      
                      <div className="flex justify-between items-center text-[10px] font-mono border-t border-slate-800 pt-3">
                        <span className="text-slate-500">Latitude: {selectedReport.latitude}</span>
                        <span className="text-slate-500">Longitude: {selectedReport.longitude}</span>
                      </div>

                      <button
                        onClick={() => setActiveTab("dashboard")}
                        className="w-full bg-slate-950 hover:bg-slate-800 text-slate-300 hover:text-white border border-slate-800 font-semibold text-xs py-2 rounded-xl cursor-pointer transition-colors"
                      >
                        Inspect in Core Dashboard
                      </button>
                    </div>
                  )}
                </div>
